...
WITH first_transaction AS (SELECT customer_id, MIN(transaction_date) AS first_transaction_date, DATE_FORMAT(MIN(transaction_date), '%Y-%m') AS activate_month
...
SELECT t.customer_id, DATE_FORMAT(t.transaction_date, '%Y-%m') AS transaction_month, SUM(t.transaction_amount) AS monthly_tpv
FROM transactions t GROUP BY t.customer_id, DATE_FORMAT(t.transaction_date, '%Y-%m')), tpv_with_safra AS (
...
SELECT mt.customer_id, ft.activate_month, TIMESTAMPDIFF(MONTH, ft.first_transaction_date, STR_TO_DATE(mt.transaction_month, '%Y-%m-01')) AS safra,
        mt.monthly_tpv, ft.first_transaction_date
FROM monthly_tpv mt JOIN first_transaction ft ON mt.customer_id = ft.customer_id), totaltpv_por_activate_month AS (SELECT activate_month, SUM(monthly_tpv) AS total_tpv
...
JOIN totaltpv_por_activate_month tt ON t.activate_month = tt.activate_month
