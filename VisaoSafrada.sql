...
/* A seguir, é hora de calcular o tpv mensal por cliente, ou seja, a soma mensal dos valores de todas as transações feitas por cada cliente. 
  ...
  SELECT t.customer_id, DATE_FORMAT(t.transaction_date, '%Y-%m') AS transaction_month, SUM(t.transaction_amount) AS monthly_tpv
    FROM transactions t
    GROUP BY t.customer_id, DATE_FORMAT(t.transaction_date, '%Y-%m')),
tpv_with_safra AS (
...
/* A partir dos dados gerados na visão de TPV mensal, nós calculamos a diferença de meses entre a data da primeira transação 
  ...
  SELECT mt.customer_id, ft.activate_month, TIMESTAMPDIFF(MONTH, ft.first_transaction_date, STR_TO_DATE(mt.transaction_month, '%Y-%m-01')) AS safra,
    ...
    FROM monthly_tpv mt
    JOIN first_transaction ft ON mt.customer_id = ft.customer_id),
totaltpv_por_activate_month AS (SELECT activate_month, SUM(monthly_tpv) AS total_tpv
    FROM tpv_with_safra
    WHERE safra = 0
