...
WITH first_transaction AS (SELECT customer_id, MIN(transaction_date) AS first_transaction_date, 
           STRFTIME('%Y-%m', MIN(transaction_date)) AS activate_month
...
/* A partir dos dados gerados na visão de TPV mensal, nós calculamos a diferença de meses entre a data da primeira transação 
  ...
  Isso significa que estamos olhando para o mês em que a primeira transação ocorreu para cada cliente, como funcionaria num MDIFF = 0. */
    ...
    FROM monthly_tpv mt 
    JOIN first_transaction ft ON mt.customer_id = ft.customer_id), totaltpv_por_activate_month AS (
...
SELECT t.activate_month, t.safra, SUM(t.monthly_tpv) AS retained_tpv, 
